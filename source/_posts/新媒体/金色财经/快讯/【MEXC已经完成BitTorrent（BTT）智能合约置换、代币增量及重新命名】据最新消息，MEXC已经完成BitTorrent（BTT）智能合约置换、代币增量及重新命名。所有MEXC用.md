
---
title: '【MEXC已经完成BitTorrent（BTT）智能合约置换、代币增量及重新命名】据最新消息，MEXC已经完成BitTorrent（BTT）智能合约置换、代币增量及重新命名。所有MEXC用...'
categories: 
 - 新媒体
 - 金色财经
 - 快讯
headimg: 'https://picsum.photos/400/300?random=1219'
author: 金色财经
comments: false
date: Invalid Date
thumbnail: 'https://picsum.photos/400/300?random=1219'
---

<div>   
【MEXC已经完成BitTorrent（BTT）智能合约置换、代币增量及重新命名】据最新消息，MEXC已经完成BitTorrent（BTT）智能合约置换、代币增量及重新命名。所有MEXC用户持有的BTT旧代币已经被转换为BTT新代币，BTT旧代币被命名为“BTTOLD”，BTT新代币被命名为“BTT”。 
BitTorrent Chain 是异构链跨链互操作扩容协议，采用 POS（Proof of Stake）共识机制 ，通过侧链进行智能合约的扩展。首发支持 Ethereum 、TRON 、BSC 公链跨链。  
</div>
            